...
import (
	...
	"fmt"
	"io/ioutil"
	"loom/context" // loom context package
	"loom/indexer"
	"loom/llm"
	"path/filepath"
	"strings"
...
func TestUserReportedFilenameSearchBug(t *testing.T) {
	...
	contextManager := context.NewContextManager(index, 1500)
	...
	// First, directly create the search task exactly as reported by the user
	llmResponse := `🔍 Search for "sample.json" (including filename matches)`
	...
	if !strings.Contains(taskResponse.ActualContent, "FOUND FILES MATCHING NAME") {
		t.Logf("Expected task response to contain 'FOUND FILES MATCHING NAME', but it didn't: %s",
			truncateForTest(taskResponse.ActualContent, 200))
	}
	...
	// 4. Add this to exploration context and chat session
	...
	// This simulates a real conversation with the LLM
	...
	for _, msg := range optimizedContext {
	...
	if !foundFilenameResults {
		...
		// Debug output
		t.Logf("Original message count: %d", len(messages))
		t.Logf("Optimized message count: %d", len(optimizedContext))
