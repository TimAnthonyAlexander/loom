...
func parseEditTask(args string) *Task {
	...
	// Look for arrow notation: "file.go → content"
	arrowPattern := regexp.MustCompile(`^(.+?)\s*→\s*(.+)$`)
	matches := arrowPattern.FindStringSubmatch(args)
	
	if len(matches) == 3 {
		task.Path = strings.TrimSpace(matches[1])
		contentDesc := strings.TrimSpace(matches[2])
		
		// This is just a description, the actual content will be provided
		// in a following code block or the LLM will need to be more specific
		task.Content = contentDesc
	} else {
